...
## 為您的部署選擇正確的閘道類型
建議您只要是網域控制站符合以下所列的大小表格，就盡可能使用 ATA 輕量型閘道，而不是使用 ATA 閘道。
除非您的網域控制站不符合 [ATA 輕量型閘道大小表格](#ata-lightweight-gateway-sizing)中的需求，否則 ATA 輕量型閘道可以且應該涵蓋大多數網域控制站。
在下列案例範例中，ATA 輕量型閘道應該涵蓋所有網域控制站︰

-   分支網站
-   來自任何 IaaS 廠商的虛擬網域控制站


## ATA 輕量型閘道大小
建議您只要是網域控制站符合此處所列的大小表格，就盡可能使用 ATA 輕量型閘道，而不是使用 ATA 閘道。
...
&#42;&#42;&#42;此網域控制站已安裝的記憶體總數。
> [!NOTE]   如果網域控制站沒有 ATA 輕量型閘道所需的必要資源數量，網域控制站的效能不會受到影響，但 ATA 輕量型閘道可能無法如預期般運作。
...
ATA 輕量型閘道可以涵蓋大多數網域控制站，因此應該根據以上的 ATA 輕量型閘道大小表格進行規劃。

如果仍然需要 ATA 閘道，以下是需要多少 ATA 閘道的考量︰<br>
...
7.  展開**網路介面卡**，然後選取 [封包/秒] 並選取適當的執行個體。 如果您不確定，您可以選取 [&lt;所有執行個體&gt;]，然後按一下 [新增] 和 [確定]。
    ...
    > [!NOTE] 若要這樣做，請在命令列執行 `ipconfig /all` 以查看介面卡和設定的名稱。
