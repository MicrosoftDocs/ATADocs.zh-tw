...
若基於某些原因而無法使用 ATA 調整大小工具，請以極短的收集間隔 (大約 5 秒)，手動收集 24 小時內所有網域控制站的 packet/sec 計數器資訊。 然後，針對每個網域控制站，計算每日平均和最繁忙期間 (15 分鐘) 平均。
下列各節提供有關如何從一個網域控制站收集 [packets/sec] 計數器的指示。
...
&#42;&#42;平均數目 (尖峰數目)
> [!NOTE]
> - ATA 中心每秒最多可以處理來自所有受監視網域控制站的1百萬個封包。 在某些環境中，相同的 ATA 中心可以處理高於1百萬的整體流量，而某些環境可能會超過 ATA 容量。 請與我們聯絡， azureatpfeedback@microsoft.com 以取得規劃和預估大型環境的協助。
...
> - 以虛擬機器的形式執行中心時 (VM) 中心需要將所有記憶體配置給 VM。 如需以虛擬機器的形式執行 ATA 中心的詳細資訊，請參閱 [Ata 中心需求](ata-prerequisites.md#dynamic-memory)
...
> - 以虛擬機器的形式執行閘道時 (VM) 閘道需要將所有記憶體配置給 VM。 如需以虛擬機器的形式執行 ATA 閘道的詳細資訊，請參閱易失 [儲存體需求](ata-prerequisites.md#dynamic-memory)) 
...
- **連接埠鏡像**  
埠鏡像考慮可能需要您針對每個資料閘道或分支網站部署多個 ATA 閘道。
- **容量**  
