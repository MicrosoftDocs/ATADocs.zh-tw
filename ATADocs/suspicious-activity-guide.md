...
如有疑問或意見反應，請聯絡 ATA 小組，網址為 [ATAEval@microsoft.com](mailto:ATAEval@microsoft.com) 。
...
設定[Active Directory 的特殊許可權存取管理（](https://docs.microsoft.com/microsoft-identity-manager/pam/privileged-identity-management-for-active-directory-domain-services)如果適用的話）。
...
在此偵測中，當 ATA 偵測到大量的簡單繫結驗證時，便會觸發警示。 這可能是在許多使用者之間以一組小型密碼*水準*進行。或「*垂直*」：只有幾個使用者會有一組大量的密碼;或這兩個選項的任意組合。
...
[複雜且長的密碼](https://docs.microsoft.com/windows/device-security/security-policy-settings/password-policy)提供必要的第一層安全性，以防範暴力密碼破解攻擊。
...
2.  黃金票證 - 在[黃金票證](#golden-ticket)警示中，相較於先前學到的行為，來自來源電腦的 TGS_REQ (服務要求) 訊息的 TGT 欄位加密方法已降級。 這不是依據時間異常偵測 (如同其他黃金票證偵測)。 此外，ATA 未偵測到與先前服務要求建立關聯的任何 Kerberos 驗證要求。
...
2.  黃金票證-在 Excel 試算表中，移至 [**網路活動**] 索引標籤。您會看到相關的降級欄位為 [**要求票證加密類型**]，而 [**來源電腦支援的加密類型**] 列出較強的加密方法。
...
3.  越過-雜湊–在 Excel 試算表中，移至 [**網路活動**] 索引標籤。您會看到相關的降級欄位為 **[加密時間戳記加密類型]** 和 [**來源電腦支援的加密類型**] 包含更強的加密方法。
...
根據 [KRBTGT Account Password Reset Scripts now available for customers](https://blogs.microsoft.com/microsoftsecure/2015/02/11/krbtgt-account-password-reset-scripts-now-available-for-customers/) (客戶現在可以使用 KRBTGT 帳戶密碼重設指令碼) 中的指導，使用 [重設 KRBTGT 帳戶密碼/金鑰工具](https://gallery.technet.microsoft.com/Reset-the-krbtgt-account-581a9e51)，變更 Kerberos 票證授權票證 (KRBTGT) 密碼兩次。 重設 KRBTGT 兩次會使此網域中的所有 Kerberos 票證失效，因此請事先規劃再這麼做。  
此外，由於建立黃金票證需要網域管理員權限，因此請實作 [雜湊傳遞建議](https://www.microsoft.com/download/details.aspx?id=36036)。
...
您可以利用 [AD ACL 掃描程式](https://blogs.technet.microsoft.com/pfesweplat/2013/05/13/take-control-over-ad-permissions-and-the-ad-acl-scanner-tool/)   或建立 Windows PowerShell 腳本，以判斷網域中具有這些許可權的人員。
...
請確定所有具有 Windows Server 2012 R2 之作業系統的網域控制站都已與 [kb3011780 一起](https://support.microsoft.com/help/2496930/ms11-013-vulnerabilities-in-kerberos-could-allow-elevation-of-privilege)一起安裝   ，而且所有成員伺服器和最多 2012 R2 的網域控制站都已與已更新 kb2496930 保持在最新狀態。 如需詳細資訊，請參閱 [銀 pac](https://technet.microsoft.com/library/security/ms11-013.aspx)   和 [偽造的 pac](https://technet.microsoft.com/library/security/ms14-068.aspx)。
...
您可以停用區域傳輸，或將區域傳輸僅限於指定的 IP 位址，來保護內部 DNS 伺服器，以防止發生使用 DNS 探查。 如需限制區域轉送的詳細資訊，請參閱[限制區域傳輸](https://technet.microsoft.com/library/ee649273(v=ws.10).aspx)。
...
3. 如果針對這兩個問題的答案皆為否，則應將此活動視為真的有問題。 試著透過檢查電腦和帳戶設定檔，來找出嘗試的來源。 按一下來源電腦或帳戶以移至其設定檔頁面。 檢查這些嘗試的期間所發生的事件，並搜尋不尋常的活動，例如當時登入的使用者，以及被存取的資源有哪些。
...
2. 執行特殊許可權 [存取](https://technet.microsoft.com/windows-server-docs/security/securing-privileged-access/securing-privileged-access)   ，只允許強化的電腦連線到系統管理員的網域控制站。
...
3.  由某些勒索軟體所控制的資料有時候可被解密。 只有當使用者尚未重新啟動或關閉電腦時，才能進行解密。 如需詳細資訊，請參閱 [Wanna Cry Ransomware](https://answers.microsoft.com/en-us/windows/forum/windows_10-security/wanna-cry-ransomware/5afdb045-8f36-4f55-a992-53398d21ed07?auth=1) (Wanna Cry 勒索軟體)
