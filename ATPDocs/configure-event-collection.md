...
- 將 SIEM 或 Syslog 伺服器設定為轉寄所有必要事件至 Azure ATP 獨立感應器的其中一個 IP 位址。 如需有關如何設定 SIEM 的詳細資訊，請參閱您的 SIEM 線上說明或每部 SIEM 伺服器之特定格式需求的技術支援選項。
...
- 欄位依序說明如下︰
    1. RsaSA 常數 (必須出現)。
    2. 實際事件的時間戳記 (請確定其不是抵達 SIEM 或傳送至 ATP 的時間戳記)。 精確度最好在毫秒以內，這很重要。
    3. Windows 事件識別碼
    4. Windows 事件提供者名稱
    5. Windows 事件記錄檔名稱
...
- 事件的_延伸_部分必須有下列索引鍵︰
  ...
  - rt = 實際事件的時間戳記 (請確定其不是抵達 SIEM 或傳送至 ATP 的時間戳記)。 精確度最好在毫秒以內，這很重要。
  - cat = Windows 事件記錄檔名稱
  - shost = 來源主機名稱
...
- 下列索引鍵必須存在且具有值︰
  ...
  - TimeGenerated = 實際事件的時間戳記 (請確定其不是抵達 SIEM 或傳送至 ATP 的時間戳記)。 必須符合 yyyyMMddHHmmss.FFFFFF 的格式，精確度最好在毫秒以內，這很重要。
  - ComputerName = 來源主機名稱
...
TimeGenerated 是實際事件的時間戳記 (請確定其不是抵達 SIEM 或傳送至 ATP 的時間戳記)。 必須符合 yyyyMMddHHmmss.FFFFFF 的格式，精確度最好在毫秒以內，這很重要。
